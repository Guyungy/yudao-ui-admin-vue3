...
export const UserInfoApi = {
  ...
  getUserInfoPage: async (params: any) => {
    return await request.get({ url: `/mail/user-info/page`, params })
  },

  // 查询用户信息详情
  getUserInfo: async (id: number) => {
    return await request.get({ url: `/mail/user-info/get?id=` + id })
  },

  // 新增用户信息
  createUserInfo: async (data: UserInfo) => {
    return await request.post({ url: `/mail/user-info/create`, data })
  },

  // 修改用户信息
  updateUserInfo: async (data: UserInfo) => {
    return await request.put({ url: `/mail/user-info/update`, data })
  },

  // 删除用户信息
  deleteUserInfo: async (id: number) => {
    return await request.delete({ url: `/mail/user-info/delete?id=` + id })
  },

  /** 批量删除用户信息 */
  deleteUserInfoList: async (ids: number[]) => {
    return await request.delete({ url: `/mail/user-info/delete-list?ids=${ids.join(',')}` })
  },

  // 导出用户信息 Excel
  exportUserInfo: async (params) => {
    return await request.download({ url: `/mail/user-info/export-excel`, params })
  },
}